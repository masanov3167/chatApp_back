...
const joiCustomErrorMessages = (label?: string) => {
  const res = {
    "string.base": `Iltimos, to'g'ri ${label || "{#label}"}ni kiriting.`,
    "string.empty": `${
      label || "{#label}"
    } maydoni bo'sh bo'lishi mumkin emas.`,
    "string.max": `${
      label || "{#label}"
    } uchun kiritish chegarasi {#limit} belgidan oshmasligi kerak.`,
...
export default joiCustomErrorMessages;
